...
cp /opt/buster/*.so /workspace/buster/
cp /opt/bullseye/*.so /workspace/bullseye/
exit
```
...
```bash
rsync -e ssh pi@<REMOTE>:/home/pi/workspace/buster/*.so ./app/buster/
rsync -e ssh pi@<REMOTE>:/home/pi/workspace/bullseye/*.so ./app/bullseye/
```
